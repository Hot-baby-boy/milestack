...
function NavItem({ href, label, icon, exact, badge, onClick }: {
  ...
  return (
    <Link href={href} onClick={onClick}
      className={`flex items-center gap-3 rounded-[9px] px-3 py-[10px] text-sm font-medium transition-all duration-150 ${
        active
          ? "bg-emerald-500/[0.16] text-white"
          : "text-slate-400 hover:bg-white/5 hover:text-white"
      }`}
    >
      <svg
        className={`h-[17px] w-[17px] flex-shrink-0 ${active ? "text-emerald-400" : ""}`}
...
export function AppSidebar({
  displayName, role, initials, unreadCount, disputeCount,
}: {
  ...
  const content = (
    <div className="flex h-full flex-col overflow-y-auto">
      <Link href="/dashboard" onClick={close}
        className="mb-6 flex items-center gap-2.5 px-[10px] pt-2 pb-6 text-[16.5px] font-extrabold tracking-tight text-white"
      >
        <LogoMark/>
        Milestack
      </Link>
      ...
      <nav className="flex flex-1 flex-col gap-[2px]">
        ...
        <div className="my-3.5 h-px bg-white/[0.08] mx-1"/>
        {NAV_BOTTOM.map(item => (
          <NavItem key={item.href + item.label} {...item}
  ...
  return (
    <>
      {/* Desktop */}
      <aside className="hidden w-[240px] flex-shrink-0 bg-gradient-to-b from-[#16223C] via-[#0F172A] to-[#060A14] p-[22px_14px] lg:flex lg:flex-col lg:sticky lg:top-0 lg:h-screen">
        {content}
      </aside>

      {/* Mobile top bar */}
      <div className="sticky top-0 z-40 flex h-[60px] items-center justify-between border-b border-slate-200 bg-white px-4 lg:hidden">
        <Link href="/dashboard" className="flex items-center gap-2 text-base font-extrabold text-slate-900">
          <LogoMark/>
          <span className="text-[#0F172A]">Milestack</span>
        </Link>
      ...
      {open && (
        <>
          ...
          <aside className="fixed inset-y-0 left-0 z-50 w-[260px] overflow-y-auto bg-gradient-to-b from-[#16223C] via-[#0F172A] to-[#060A14] p-[22px_14px] lg:hidden">
            {content}
          </aside>
        </>
      )}
